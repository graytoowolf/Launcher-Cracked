...
BsYggdrasil::BsYggdrasil(AccountData *data, QObject *parent)
    : AccountTask(data, parent)
{
    changeState(AccountTaskState::STATE_CREATED);
}

void BsYggdrasil::sendRequest(QUrl endpoint, QByteArray content)
{
    ...
    connect(m_netReply, &QNetworkReply::downloadProgress, this, &BsYggdrasil::refreshTimers);
    connect(m_netReply, &QNetworkReply::sslErrors, this, &BsYggdrasil::sslErrors);
    timeout_keeper.setSingleShot(true);
    timeout_keeper.start(timeout_max);
    counter.setSingleShot(false);
    counter.start(time_step);
    progress(0, timeout_max);
    connect(&timeout_keeper, &QTimer::timeout, this, &BsYggdrasil::abortByTimeout);
    connect(&counter, &QTimer::timeout, this, &BsYggdrasil::heartbeat);
}

void BsYggdrasil::executeTask()
{
}

void BsYggdrasil::refresh()
{
    start();
    /*
     * {
    ...
    */
    ...
    QUrl reqUrl(m_data->yggurl() + "authserver/refresh");
    QByteArray requestData = doc.toJson();

    sendRequest(reqUrl, requestData);
}

void BsYggdrasil::login(QString password)
{
    start();
    /*
     * {
...
void BsYggdrasil::refreshTimers(qint64, qint64)
{
    timeout_keeper.stop();
    timeout_keeper.start(timeout_max);
...
void BsYggdrasil::heartbeat()
{
    count += time_step;
    progress(count, timeout_max);
}

bool BsYggdrasil::abort()
{
    progress(timeout_max, timeout_max);
    // TODO: actually use this in a meaningful way
    m_aborted = BsYggdrasil::BY_USER;
    m_netReply->abort();
    return true;
}

void BsYggdrasil::abortByTimeout()
{
    progress(timeout_max, timeout_max);
    // TODO: actually use this in a meaningful way
    m_aborted = BsYggdrasil::BY_TIMEOUT;
    m_netReply->abort();
}

void BsYggdrasil::sslErrors(QList<QSslError> errors)
{
    int i = 1;
...
void BsYggdrasil::processResponse(QJsonObject responseData)
{
...
void BsYggdrasil::processReply()
{
    ...
    // If the response code was not 200, then BsYggdrasil may have given us information
    // about the error.
...
void BsYggdrasil::processError(QJsonObject responseData)
{
    ...
    else
    {
        ...
        changeState(AccountTaskState::STATE_FAILED_HARD, tr("An unknown BsYggdrasil error occurred."));
    }
}
