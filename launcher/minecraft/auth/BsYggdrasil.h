...
/**
 * A BsYggdrasil task is a task that performs an operation on a given mojang account.
 */
class BsYggdrasil : public AccountTask
{
    Q_OBJECT
public:
    explicit BsYggdrasil(
        AccountData *data,
        QObject *parent = 0);
    virtual ~BsYggdrasil() = default;
...
protected:
     ...
     * If BsYggdrasil gave an error response, it should call setError() first, and then return false.
     ...
     * The default implementation will read data from BsYggdrasil's standard error response format and set it as this task's Error.
